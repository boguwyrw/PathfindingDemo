...
namespace pathfinding.demo
{
    public class GridMapGenerator : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            GameManager.Instance.SetInfoText("Set player position");
        }
            ...
            if (Physics.Raycast(ray, out RaycastHit hit))
            {
                if (Input.GetMouseButtonDown(0))
                {
                    ...
                    if (tile.IsTileTraversable && !isMarkAsStart)
                    {
                        ...
                        playerGO = Instantiate(GameManager.Instance.Player, new Vector3(tile.TileX, 0.5f, tile.TileZ), Quaternion.identity);
                        GameManager.Instance.SetInfoText("Set enemy position");
                    }
                    else if (tile.IsTileTraversable && isMarkAsStart && !isMarkAsGoal)
                    {
                        ...
                        Instantiate(GameManager.Instance.Enemy, new Vector3(tile.TileX, 0.5f, tile.TileZ), Quaternion.identity);
                        Player playerScript = playerGO.GetComponent<Player>();
                        bool outOfRange = playerScript.AvailableMoveTiles.Contains(goal);
                        if (!outOfRange)
                        {
                            GameManager.Instance.SetInfoText("Out of range");
                        }
        ...
        private void GridMap(MapConfig data)
        {
        ...
        private Vector3 TileWorldPosition(int x, int z)
        {
        ...
        public void FindPathButton()
        {
            ...
            {
                GameManager.Instance.SetInfoText("No path!");
            }
